...
\begin{箇条書}
\item \textbf{チョッパ方式（非絶縁型）}：スイッチで刻んで平均値を制御する。降圧・昇圧・昇降圧チョッパ。本章で扱う
\item \textbf{チョッパ方式（絶縁型）}：同じくスイッチで刻むが，変圧器を挟んで入力と出力を絶縁する。フォワード・フライバックなど（\ref{chap6}章）
\item \textbf{リニア方式}：余分な電圧を抵抗として連続的に落とす（\ref{chap7}章）
\end{箇条書}

はじめの2つは「スイッチで刻む」という点で同じ仲間であり，違いは変圧器を挟むかどうかだけである。
...
刻むという点では絶縁型も変わらない。\ref{chap6}章のフォワードもフライバックも，本章のチョッパに変圧器を挟んだものである。
ただし呼び名には慣習があり，日本語では「チョッパ」を非絶縁の3つに使い，絶縁型は「コンバータ」と呼ぶ
（英語では chopper は方式そのものを指し，回路は buck / boost / buck-boost converter，forward / flyback converter と呼ぶ）。
本書もこの慣習に従うが，\textbf{チョッパとは方式の名前であって，絶縁の有無とは関係がない}。
...
\end{center}
\caption{過渡状態から定常状態へ（次節の降圧チョッパを電源投入から計算した波形。$V_{\mathrm{in}} = 10$\,V，$D = 0.5$，$f = 1$\,kHz）。
...
\rev{デューティ比とボルト秒平衡 ― 道具はこの2つで足りる。次節から，3つのチョッパ回路を上の手順どおりに順に解析していく。}
...
\end{center}
\caption{降圧チョッパ。(a)回路構成，(b)オン期間，(c)オフ期間の等価回路。灰色は電流が流れない部分，\rev{太い矢印は電流の経路を表す}}
\label{fig5.3}
\end{figure}
...
\rev{降圧チョッパの解析はこれで一通り済んだ。同じ4つの素子の並べ方を変えるだけで，入力より高い電圧が作れる ― それが次節の昇圧チョッパである。}
...
（\index{ぶーすとこんばーた@ブーストコンバータ}ブーストコンバータ，boost converter）である（\FIGref{fig5.5}(a)）。素子は降圧チョッパと同じ4つだが，配置が違う。
...
定常状態の波形を\FIGref{fig5.6}に示す。降圧チョッパと違い，$i_L$の平均は\textbf{入力電流}$I_{\mathrm{in}}$である。
...
\rev{降圧チョッパにも昇圧チョッパにも，電源が負荷へ直接エネルギーを送る期間があった。次節では，その期間をなくし，蓄えと放出だけで電圧を作る昇降圧チョッパを見る。}
...
\item \rev{昇圧・昇降圧チョッパでは，オン期間の$v_L = V_{\mathrm{in}}$から$L = V_{\mathrm{in}} D/(f\,\mathit{\Delta}I_L)$とし，
...
\item \rev{昇圧・昇降圧チョッパでは，2.の式\ref{eq5.25}の代わりに式\ref{eq5.25b}を使う（次項）}
\end{enumerate}
\end{screen}
...
\subsection{昇圧・昇降圧チョッパの出力キャパシタ}

\rev{式\ref{eq5.24}は，$C$に三角波の電流が流れ込む降圧チョッパにだけ使える式である。
昇圧チョッパと昇降圧チョッパでは事情が違う。\ref{fig5.5}(b)・\ref{fig5.7}(b)で見たとおり，
...
\begin{equation}
\mathit{\Delta}V_{\mathrm{out}} = \frac{I_{\mathrm{out}}\,DT}{C} = \frac{I_{\mathrm{out}}\, D}{f\, C} \qquad （昇圧・昇降圧チョッパ）
\label{eq5.24b}
\end{equation}
\rev{となる。式\ref{eq5.24}と違ってリプル電流$\mathit{\Delta}I_L$は現れず，負荷電流$I_{\mathrm{out}}$そのものが効く。
同じ負荷・同じ$f$・同じ$C$なら，降圧チョッパのリプル電圧は$\mathit{\Delta}I_L/8$に比例し，こちらは$I_{\mathrm{out}}D$に比例する。
$\mathit{\Delta}I_L$が$I_{\mathrm{out}}$の2割，$D = 0.5$なら，その比は20倍 ― 昇圧・昇降圧チョッパの出力キャパシタの負担はそれだけ重い。
キャパシタを決める手順は，前の手順ボックスの2.を式\ref{eq5.24b}に置き換えて}
\begin{equation}
C = \frac{I_{\mathrm{out}}\, D}{f\, \mathit{\Delta}V_{\mathrm{out}}} \qquad （昇圧・昇降圧チョッパ）
\label{eq5.25b}
\end{equation}
...
そのうえで，章末問題\ref{ex5.2}の昇圧チョッパ（$12$\,V→$48$\,V・$0.5$\,A，$f = 100$\,kHz）で，
...
\item \label{ex5.6}
（LTspiceで確かめよ）\rev{配布モデル\texttt{chapter05/buck\_chopper.asc}}の降圧チョッパ（$V_{\mathrm{in}} = 10$\,V，$D = 0.5$，$f = 1$\,kHz，$L = 30$\,mH，

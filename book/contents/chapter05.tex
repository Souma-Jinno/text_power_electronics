...
入力電圧$V_{\mathrm{in}}$より低い出力電圧$V_{\mathrm{out}}$を作る回路である（\FIGref{fig5.2}(a)）。登場する素子は4つで，役割がはっきり分かれている。
...
\textbf{オン期間}（$0 \le t < DT$）は，Sが導通し，電流は電源→S→$L$→負荷の経路を流れる（\ref{fig5.2}(b)）。
...
行き場を失った電流はダイオードを押し開け，$\mathrm{D}$→$L$→負荷の経路で流れ続ける（\ref{fig5.2}(c)）。
...
定常状態の波形を\FIGref{fig5.3}に示す。$v_L$は$V_{\mathrm{in}}-V_{\mathrm{out}}$と$-V_{\mathrm{out}}$の2値の矩形波，
...
（\index{ぶーすとこんばーた@ブーストコンバータ}ブーストコンバータ，boost converter）である（\FIGref{fig5.4}(a)）。素子は降圧チョッパと同じ4つだが，配置が違う。
...
\textbf{オン期間}（$0 \le t < DT$）は，Sが導通して電源→$L$→Sのループができる（\ref{fig5.4}(b)）。このループのキルヒホッフの電圧則は$V_{\mathrm{in}} = v_L$，すなわち
\begin{equation}
v_L = V_{\mathrm{in}}
\label{eq5.10}
\end{equation}
...
電源→$L$→$\mathrm{D}$→負荷の経路ができる（\ref{fig5.4}(c)）。キルヒホッフの電圧則は$V_{\mathrm{in}} = v_L + V_{\mathrm{out}}$，すなわち
\begin{equation}
v_L = V_{\mathrm{in}} - V_{\mathrm{out}}
\label{eq5.11}
\end{equation}
...
定常状態の波形を\FIGref{fig5.5}に示す。降圧チョッパと違い，$i_L$の平均は\textbf{入力電流}$I_{\mathrm{in}}$である。
...
（\index{ばっくぶーすとこんばーた@バックブーストコンバータ}バックブーストコンバータ，buck-boost converter）である（\FIGref{fig5.6}(a)）。素子はやはり同じ4つで，
...
\textbf{オン期間}（$0 \le t < DT$）は，Sが導通して電源→S→$L$のループができ（\ref{fig5.6}(b)），
\begin{equation}
v_L = V_{\mathrm{in}}
\label{eq5.15}
\end{equation}
...
\textbf{オフ期間}（$DT \le t < T$）は，Sが切れ，電流を続けようとする$L$が$\mathrm{D}$を押し開けて，$L$→負荷→$\mathrm{D}$のループができる（\ref{fig5.6}(c)）。
\rev{出力電圧$V_{\mathrm{out}}$を，これまでの回路と同じ向き（\ref{fig5.6}(a)の極性表示どおり，上の端子を正）で測ると，理想ダイオードを介して$L$と負荷が並列につながるから}
\begin{equation}
v_L = V_{\mathrm{out}}
\label{eq5.16}
\end{equation}
...
\begin{注意}
\rev{式\ref{eq5.18}の負号は，\ref{fig5.6}(a)の極性表示どおり上の端子を正として測った$V_{\mathrm{out}}$が負の値になる，
...
ここからは設計の話に入る。\ref{fig5.3}で見たとおり，インダクタ電流は平均値のまわりを三角波状に増減する。
...
\label{eq5.22}
\end{equation}
すなわち，$i_L$から平均値を除いた三角波の変動分がそっくり$C$に流れ込む（\ref{fig5.3}の3段目）。
...
\FIGref{fig5.7}を見ながら，1周期の間の電圧の揺れ幅を求めよう。キャパシタの電圧は$i_C > 0$の間だけ上がり続ける。
...
昇圧チョッパと昇降圧チョッパでは事情が違う。\ref{fig5.4}(b)・\ref{fig5.6}(b)で見たとおり，
...
ここまで「インダクタ電流は常に正」と仮定してきた。この仮定が崩れるのはどんなときか。\ref{fig5.3}の$i_L$の谷の値は
\begin{equation}
...
やがて谷が0に触れ，さらに負荷が軽くなると電流が0に張り付く期間が現れる（\FIGref{fig5.8}）。
...
\textbf{Sも$\mathrm{D}$も導通せず$i_L = 0$のまま止まっている第3の期間}をもつようになる（\ref{fig5.8}(c)）。この期間は$v_L = 0$だから，ボルト秒平衡の式\ref{eq5.5}は
\begin{equation*}
...
つまり\textbf{同じリプルを許すなら，$f$を10倍にすれば$L$も$C$も10分の1で済む}（\FIGref{fig5.9}）。インダクタとキャパシタは電源の体積の大半を占める部品だから，

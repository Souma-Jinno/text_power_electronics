...
\rev{出力電圧$V_{\mathrm{out}}$を，これまでの回路と同じ向き（\ref{fig5.5}(a)の極性表示どおり，上の端子を正）で測ると，理想ダイオードを介して$L$と負荷が並列につながるから}
\begin{equation}
v_L = V_{\mathrm{out}}
\label{eq5.16}
\end{equation}
...
\begin{注意}
\rev{式\ref{eq5.18}の負号は，\ref{fig5.5}(a)の極性表示どおり上の端子を正として測った$V_{\mathrm{out}}$が負の値になる，
...
昇圧チョッパと昇降圧チョッパでは事情が違う。\ref{fig5.3}(b)・\ref{fig5.5}(b)で見たとおり，

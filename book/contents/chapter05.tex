...
本章の3つの回路（降圧・昇圧・昇降圧コンバータ）は部品数こそ少ないが，
...
%% [対応済 2026-09-08] 著者指摘「チョッパという表現はしない」。この囲みは章の入口で，
%%   チョッパの定義（5.1.1節）より前に出る。しかも「非絶縁型のチョッパ回路」という言い方は，
%%   5.1.1節で正した「チョッパ＝方式の名前で絶縁の有無とは無関係」と食い違う。
%% [再対応 2026-09-08] 著者指示「チョッパは使わないでね」により，本章では「チョッパ」の語を使わず，
%%   降圧／昇圧／昇降圧コンバータ・スイッチング方式に全面的に改めた（上の経緯は記録として残す）。
本章はその第1歩，トランスを使わない\index{ひぜつえんがた@非絶縁型}\textbf{非絶縁型}である。
...
%% [対応済 2026-09-08] 著者指摘「非絶縁型チョッパ回路がありますが，これは絶縁型でもチョッパ回路ですよね？」
%%   旧版は「非絶縁型チョッパ回路／絶縁型コンバータ／リニアレギュレータ」と並べており，
%%   「チョッパ＝非絶縁，コンバータ＝絶縁」と読めてしまっていた。しかし直後の定義
%%   （スイッチで刻んで平均値を制御する）は絶縁型にも当てはまり，6章自身が
%%   「降圧チョッパのスイッチと還流ダイオードの間に変圧器を挟んだ」と書いている。
%%   軸が2本（刻むか連続か／絶縁するかしないか）混ざっていたのが原因。
%%   英語の教科書も chopper を「スイッチング方式そのもの」の意味で使い（Erickson:
%%   "switched-mode, or chopper, circuits"），分類は non-isolated / transformer-isolated
%%   の軸で立てている。著者指示「そのルールに倣ってくれない？」によりその形に改めた。
\begin{箇条書}
\item \textbf{スイッチング方式（非絶縁型）}：スイッチで刻んで平均値を制御する。降圧・昇圧・昇降圧コンバータ。本章で扱う
\item \textbf{スイッチング方式（絶縁型）}：同じくスイッチで刻むが，変圧器を挟んで入力と出力を絶縁する。フォワード・フライバックなど（\ref{chap6}章）
\item \textbf{リニア方式}：余分な電圧を抵抗として連続的に落とす（\ref{chap7}章）
\end{箇条書}

%% [対応済 2026-09-08] 著者指示「チョッパは使わないでね」。旧版はここで「チョッパ（chopper）」を定義し，
%%   日本語の慣習（非絶縁＝チョッパ／絶縁＝コンバータ）を説明していたが，語そのものを使わないので慣習の説明も不要になった。
%%   かわりに「スイッチング方式とは何か」と「非絶縁型と絶縁型の違いは変圧器を挟むかどうかだけ」を述べる形に書き直した。
%%   呼び名は英語の教科書に倣い，降圧／昇圧／昇降圧コンバータ（buck / boost / buck-boost converter）とする。
はじめの2つに共通する\index{すいっちんぐほうしき@スイッチング方式}\textbf{スイッチング方式}とは，半導体スイッチで入力電圧を高速に刻み，その平均値を制御する方式である。
理想的にはスイッチは「電圧ゼロで電流を流す」か「電流ゼロで電圧を受け持つ」かのどちらかの状態しか取らないため，原理的に損失がない。
効率よく，かつ小さな部品で大きな電力を扱えることがスイッチング方式の利点であり，現代の電源回路の主流である。

本章で扱う非絶縁型の3つの回路は，スイッチで刻んだ電圧をインダクタとキャパシタで受け止めて出力にする，最も基本的な形である。
\ref{chap6}章のフォワードもフライバックも，これらの回路に変圧器を挟んだものにすぎない。
\textbf{非絶縁型と絶縁型の違いは変圧器を挟むかどうかだけ}であり，刻んで平均値を制御するという原理も，本章で身につける解析の手順も，どちらにもそのまま当てはまる。
%% [著者確認中 2026-09-08] 「文献によっては降圧チョッパ・昇圧チョッパ・昇降圧チョッパともいう」の1行を初出に1か所だけ残すかどうかは
%%   著者の判断待ち。入れる場合の位置は 5.2節冒頭の降圧コンバータの定義文（\index{こうあつこんばーた@...}の行）の直後。そこにも目印コメントを置いた。

スイッチングの時間配分を表すのが次のデューティ比である。

\begin{定義}[デューティ比]
\label{def5.1}
スイッチを周期$T$（\index{すいっちんぐしゅうき@スイッチング周期}スイッチング周期）でオン・オフし，そのうちオンの時間を$T_{\mathrm{on}}$とするとき\rev{（\FIGref{fig5.1}）}，
\begin{equation}
...
\end{center}
\caption{デューティ比。スイッチのオン・オフを周期$T$で繰り返し，そのうちオンの時間が$T_{\mathrm{on}}$である。$D = T_{\mathrm{on}}/T$は1周期のうちオンの時間が占める割合を表す。(a)$D$が小さいとき，(b)$D$が大きいとき}
\label{fig5.1}
\end{figure}

本章の主役はこの$D$である。後で示すように，3つの回路の出力電圧はいずれも$D$だけで決まり，$D$を変えることで出力を連続的に制御できる。

\subsection{定常状態とボルト秒平衡 ― 4章の引き継ぎ}

変換回路の解析の鍵は，4章で学んだインダクタの性質である。インダクタの電圧$v_L$と電流$i_L$の関係
\begin{equation}
...
\rev{この移り変わりを\FIGref{fig5.2}に示す。電源を入れた直後は$v_L$の正の面積が負の面積より大きく，$i_L$は周期ごとに階段状に持ち上がっていくが，やがて正負の面積が等しくなると，$i_L$は1周期でもとの値に戻り，毎周期同じ三角波を繰り返すだけになる。}

\begin{figure}[ht]
\begin{center}
\includegraphics[clip]{figures/fig5.2.eps}
\end{center}
\caption{過渡状態から定常状態へ（次節の降圧コンバータを電源投入から計算した波形。$V_{\mathrm{in}} = 10$\,V，$D = 0.5$，$f = 1$\,kHz）。
過渡状態では$v_L$の正の面積が負の面積より大きく，$i_L$は1周期ごとに増える。
定常状態では正負の面積が等しく（ボルト秒平衡），$i_L$は1周期でもとの値に戻る}
\label{fig5.2}
\end{figure}
...
\rev{デューティ比とボルト秒平衡 ― 道具はこの2つで足りる。次節から，3つの回路を上の手順どおりに順に解析していく。}
...
%% [著者確認中 2026-09-08] 「文献によっては降圧チョッパともいう」の1行を残す場合はこの直後（buck converter の括弧の後）に入れる。
%%   例：「...buck converter。文献によっては降圧チョッパともいう）は，」。5.1.1節の目印コメントも参照。
\index{こうあつこんばーた@降圧コンバータ}\textbf{降圧コンバータ}（\index{ばっくこんばーた@バックコンバータ}バックコンバータ，buck converter）は，
入力電圧$V_{\mathrm{in}}$より低い出力電圧$V_{\mathrm{out}}$を作る回路である（\FIGref{fig5.3}(a)）。登場する素子は4つで，役割がはっきり分かれている。
...
\end{center}
\caption{降圧コンバータ。(a)回路構成，(b)オン期間，(c)オフ期間の等価回路。灰色は電流が流れない部分，\rev{太い矢印は電流の経路を表す}}
\label{fig5.3}
\end{figure}
...
\textbf{オン期間}（$0 \le t < DT$）は，Sが導通し，電流は電源→S→$L$→負荷の経路を流れる（\ref{fig5.3}(b)）。
...
\label{eq5.6}
\end{equation}
である。降圧コンバータでは$V_{\mathrm{in}} > V_{\mathrm{out}}$だから$v_L > 0$，
...
行き場を失った電流はダイオードを押し開け，$\mathrm{D}$→$L$→負荷の経路で流れ続ける（\ref{fig5.3}(c)）。
...
\end{center}
\caption{降圧コンバータの定常状態波形。$v_L$の正負の面積が等しく（ボルト秒平衡），
$i_L$は三角波状に増減する。$i_L$の変動分はキャパシタ電流$i_C$となり，平均値$I_{\mathrm{out}}$だけが負荷へ流れる}
\label{fig5.4}
\end{figure}

定常状態の波形を\FIGref{fig5.4}に示す。$v_L$は$V_{\mathrm{in}}-V_{\mathrm{out}}$と$-V_{\mathrm{out}}$の2値の矩形波，
...
\begin{例題}[降圧コンバータの動作点]
\label{rei5.1}
入力$12$\,Vの降圧コンバータで出力$5$\,Vを得たい。スイッチング周波数を$f = 100$\,kHzとするとき，
...
\begin{問}
\item 入力$48$\,Vから出力$12$\,Vを作る降圧コンバータのデューティ比を求めよ。また，オフ期間にスイッチSの両端にかかる電圧を求めよ。
\end{問}

\rev{降圧コンバータの解析はこれで一通り済んだ。同じ4つの素子の並べ方を変えるだけで，入力より高い電圧が作れる ― それが次節の昇圧コンバータである。}
...
\textbf{いったんエネルギーを蓄え，電源に上乗せして送り出す}しかない。これを実現するのが\index{しょうあつこんばーた@昇圧コンバータ}\textbf{昇圧コンバータ}
（\index{ぶーすとこんばーた@ブーストコンバータ}ブーストコンバータ，boost converter）である（\FIGref{fig5.5}(a)）。素子は降圧コンバータと同じ4つだが，配置が違う。
...
\end{center}
\caption{昇圧コンバータ。(a)回路構成，(b)オン期間，(c)オフ期間の等価回路。オン期間は出力がダイオードで切り離され，$C$が負荷への供給を受け持つ}
\label{fig5.5}
\end{figure}

\subsection{オン期間とオフ期間の等価回路}

\textbf{オン期間}（$0 \le t < DT$）は，Sが導通して電源→$L$→Sのループができる（\ref{fig5.5}(b)）。このループのキルヒホッフの電圧則は$V_{\mathrm{in}} = v_L$，すなわち
\begin{equation}
v_L = V_{\mathrm{in}}
\label{eq5.10}
\end{equation}
...
電源→$L$→$\mathrm{D}$→負荷の経路ができる（\ref{fig5.5}(c)）。キルヒホッフの電圧則は$V_{\mathrm{in}} = v_L + V_{\mathrm{out}}$，すなわち
\begin{equation}
v_L = V_{\mathrm{in}} - V_{\mathrm{out}}
\label{eq5.11}
\end{equation}
である。昇圧コンバータでは$V_{\mathrm{out}} > V_{\mathrm{in}}$になる（すぐ後で示す）から$v_L < 0$，
...
\end{center}
\caption{昇圧コンバータの定常状態波形。インダクタ電流$i_L$の平均は入力電流
$I_{\mathrm{in}}$であり，オフ期間だけダイオード電流$i_{\mathrm{D}}$として出力へ流れる。その平均が負荷電流$I_{\mathrm{out}}$になる}
\label{fig5.6}
\end{figure}

定常状態の波形を\FIGref{fig5.6}に示す。降圧コンバータと違い，$i_L$の平均は\textbf{入力電流}$I_{\mathrm{in}}$である。
...
\begin{例題}[昇圧コンバータの動作点]
\label{rei5.2}
バッテリー電圧$5$\,Vから$12$\,Vを作る昇圧コンバータについて，(a)~デューティ比$D$，(b)~出力電流が$0.42$\,Aのときの入力電流を求めよ。損失はないものとする。
\begin{解答}
...
\end{equation*}
出力の2.4倍の電流が入力側に流れる。昇圧コンバータの入力側の配線・インダクタ・スイッチは，この大きな電流を前提に選ばなければならない。
\end{解答}
\end{例題}
...
\rev{降圧コンバータにも昇圧コンバータにも，電源が負荷へ直接エネルギーを送る期間があった。次節では，その期間をなくし，蓄えと放出だけで電圧を作る昇降圧コンバータを見る。}
...
入力より高い電圧も低い電圧も1つの回路で作れるのが\index{しょうこうあつこんばーた@昇降圧コンバータ}\textbf{昇降圧コンバータ}
（\index{ばっくぶーすとこんばーた@バックブーストコンバータ}バックブーストコンバータ，buck-boost converter）である（\FIGref{fig5.7}(a)）。素子はやはり同じ4つで，
...
\end{center}
\caption{昇降圧コンバータ。(a)回路構成，(b)オン期間，(c)オフ期間の等価回路。
エネルギーはいったんすべて$L$に蓄えられてから負荷へ渡り，出力の極性が反転する（上の端子が負になる）}
\label{fig5.7}
\end{figure}

\textbf{オン期間}（$0 \le t < DT$）は，Sが導通して電源→S→$L$のループができ（\ref{fig5.7}(b)），
\begin{equation}
v_L = V_{\mathrm{in}}
\label{eq5.15}
\end{equation}
...
\textbf{オフ期間}（$DT \le t < T$）は，Sが切れ，電流を続けようとする$L$が$\mathrm{D}$を押し開けて，$L$→負荷→$\mathrm{D}$のループができる（\ref{fig5.7}(c)）。
\rev{出力電圧$V_{\mathrm{out}}$を，これまでの回路と同じ向き（\ref{fig5.7}(a)の極性表示どおり，上の端子を正）で測ると，理想ダイオードを介して$L$と負荷が並列につながるから}
\begin{equation}
v_L = V_{\mathrm{out}}
\label{eq5.16}
\end{equation}
...
\begin{注意}
\rev{式\ref{eq5.18}の負号は，\ref{fig5.7}(a)の極性表示どおり上の端子を正として測った$V_{\mathrm{out}}$が負の値になる，
すなわち出力の\index{きょくせいはんてん@極性反転}\textbf{極性が反転}し，\textbf{実際には上の端子が負，下の端子が正になる}ことを表す。
オン期間に$L$には上から下へ電流が流れ込み，オフ期間もインダクタはこの向きの電流を続けようとする。
その電流は負荷を\textbf{下から上へ}貫くから，出力の上の端子が負になるのである。}
負電源を作れる利点でもあるが，制御回路の基準電位（グラウンド）の扱いには注意を要する。
\end{注意}

\textbf{エネルギーの流れ}：降圧コンバータではオン期間に電源から負荷へ直接電力が流れ，
昇圧コンバータではオフ期間に電源が直接負荷へ送り込んでいた。これに対して昇降圧コンバータでは，\textbf{エネルギーはいったんすべて$L$に蓄えられ，それから負荷へ渡される}。
...
\hline
降圧コンバータ & $D$ & $0 \sim V_{\mathrm{in}}$ & 入力と同じ \\
昇圧コンバータ & $\dfrac{1}{1-D}$ & $V_{\mathrm{in}}$以上 & 入力と同じ \\
昇降圧コンバータ & $-\dfrac{D}{1-D}$ & 0から昇圧域まで & 反転 \\
\hline
\end{tabular}
\end{center}
\end{table}
...
\begin{問}
\item 入力$12$\,Vの昇降圧コンバータで出力$-5$\,Vおよび$-24$\,Vを得るためのデューティ比をそれぞれ求めよ。
\end{問}

\rev{ここまでで3回路の電圧比が出そろった。次節からは設計に入り，波形の揺れ（リプル）から$L$と$C$の値を決める。}
...
ここからは設計の話に入る。\ref{fig5.4}で見たとおり，インダクタ電流は平均値のまわりを三角波状に増減する。
この振れの全幅（山から谷まで）を\index{りぷるでんりゅう@リプル電流}\textbf{リプル電流}$\mathit{\Delta}I_L$という。

$\mathit{\Delta}I_L$は波形を描くまでもなく計算できる。降圧コンバータのオン期間，電流は一定の傾き
...
\begin{screen}
\noindent\textbf{手順：リプル電流からインダクタを決める（降圧コンバータ）}\\[3pt]
\begin{enumerate}
...
\item \rev{昇圧・昇降圧コンバータでは，オン期間の$v_L = V_{\mathrm{in}}$から$L = V_{\mathrm{in}} D/(f\,\mathit{\Delta}I_L)$とし，
5.の電流の山は$i_L$の平均（昇圧では入力電流$I_{\mathrm{in}}$）に$\mathit{\Delta}I_L/2$を加えて求める}
\end{enumerate}
\end{screen}

\begin{例題}[インダクタの設計]
\label{rei5.3}
$12$\,Vから$5$\,V・$2$\,Aを作る降圧コンバータ（$f = 100$\,kHz）で，
...
\rev{$L$が決まれば，$i_L$の揺れ幅$\mathit{\Delta}I_L$も決まる。その揺れがどこへ行き，出力電圧をどれだけ揺らすか ― それが次節のキャパシタ設計である。}
...
\label{eq5.22}
\end{equation}
すなわち，$i_L$から平均値を除いた三角波の変動分がそっくり$C$に流れ込む（\ref{fig5.4}の3段目）。
...
\FIGref{fig5.8}を見ながら，1周期の間の電圧の揺れ幅を求めよう。キャパシタの電圧は$i_C > 0$の間だけ上がり続ける。
...
\begin{screen}
\noindent\textbf{手順：リプル電圧からキャパシタを決める（降圧コンバータ）}\\[3pt]
\begin{enumerate}
...
\item \rev{昇圧・昇降圧コンバータでは，2.の式\ref{eq5.25}の代わりに式\ref{eq5.25b}を使う（次項）}
\end{enumerate}
\end{screen}

\begin{例題}[キャパシタの設計]
\label{rei5.4}
例題\ref{rei5.3}の降圧コンバータ（$f = 100$\,kHz，$\mathit{\Delta}I_L = 0.4$\,A）で，
...
\subsection{昇圧・昇降圧コンバータの出力キャパシタ}

\rev{式\ref{eq5.24}は，$C$に三角波の電流が流れ込む降圧コンバータにだけ使える式である。
昇圧コンバータと昇降圧コンバータでは事情が違う。\ref{fig5.5}(b)・\ref{fig5.7}(b)で見たとおり，
オン期間はダイオードが遮断されて出力側が電源から切り離され，負荷電流$I_{\mathrm{out}}$をキャパシタの放電だけが受け持つ。
つまり$C$には三角波ではなく，オン期間に$-I_{\mathrm{out}}$，オフ期間にダイオード電流から$I_{\mathrm{out}}$を引いた値，というパルス状の電流が流れる。
オン期間$DT$の間に$C$から失われる電荷は$\mathit{\Delta}Q = I_{\mathrm{out}}\,DT$だから，リプル電圧は}
\begin{equation}
\mathit{\Delta}V_{\mathrm{out}} = \frac{I_{\mathrm{out}}\,DT}{C} = \frac{I_{\mathrm{out}}\, D}{f\, C} \qquad （昇圧・昇降圧コンバータ）
\label{eq5.24b}
\end{equation}
\rev{となる。式\ref{eq5.24}と違ってリプル電流$\mathit{\Delta}I_L$は現れず，負荷電流$I_{\mathrm{out}}$そのものが効く。
同じ負荷・同じ$f$・同じ$C$なら，降圧コンバータのリプル電圧は$\mathit{\Delta}I_L/8$に比例し，こちらは$I_{\mathrm{out}}D$に比例する。
$\mathit{\Delta}I_L$が$I_{\mathrm{out}}$の2割，$D = 0.5$なら，その比は20倍 ― 昇圧・昇降圧コンバータの出力キャパシタの負担はそれだけ重い。
キャパシタを決める手順は，前の手順ボックスの2.を式\ref{eq5.24b}に置き換えて}
\begin{equation}
C = \frac{I_{\mathrm{out}}\, D}{f\, \mathit{\Delta}V_{\mathrm{out}}} \qquad （昇圧・昇降圧コンバータ）
\label{eq5.25b}
\end{equation}
\rev{とすればよい。さらに，スイッチの切替えのたびにキャパシタ電流はインダクタ電流$i_L$の分だけ跳ぶので，
ESRによる電圧の段差も$r_C\, i_L$と大きい。昇圧・昇降圧ではESRの小さいキャパシタを選ぶことがいっそう重要になる。
この式は，次章のフライバックコンバータの出力キャパシタにもそのまま使う。}

\begin{問}
\item 降圧コンバータで$V_{\mathrm{in}} = 10$\,V，$D = 0.5$，$f = 1$\,kHz，
...
\begin{問}
\item \rev{式\ref{eq5.24b}の右辺の単位がV（ボルト）になることを確かめよ（$1\,\mathrm{F} = 1\,\mathrm{A\cdot s/V}$を使ってよい）。
そのうえで，章末問題\ref{ex5.2}の昇圧コンバータ（$12$\,V→$48$\,V・$0.5$\,A，$f = 100$\,kHz）で，
リプル電圧を$\mathit{\Delta}V_{\mathrm{out}} = 0.5$\,V（出力の約1\,\%）に抑えるための$C$を式\ref{eq5.25b}から求めよ。}
\end{問}

\rev{以上で，リプルから$L$と$C$を決める手順が3回路ともそろった。次節では，本章の仮定のうち「インダクタ電流は常に正」が崩れるとどうなるかを見る。}
...
ここまで「インダクタ電流は常に正」と仮定してきた。この仮定が崩れるのはどんなときか。\ref{fig5.4}の$i_L$の谷の値は
\begin{equation}
...
やがて谷が0に触れ，さらに負荷が軽くなると電流が0に張り付く期間が現れる（\FIGref{fig5.9}）。
ダイオードは逆向きに電流を流せないので，電流が0に達した瞬間に勝手にオフしてしまうからである。

\begin{figure}[ht]
\begin{center}
\includegraphics[clip]{figures/fig5.9.eps}
\end{center}
...
\textbf{Sも$\mathrm{D}$も導通せず$i_L = 0$のまま止まっている第3の期間}をもつようになる（\ref{fig5.9}(c)）。この期間は$v_L = 0$だから，ボルト秒平衡の式\ref{eq5.5}は
\begin{equation*}
...
\rev{軽負荷で仮定が崩れる話はここまでにして，最後に本章の設計式を俯瞰し，スイッチング周波数$f$が電源の大きさをどう決めるかを見る。}
...
つまり\textbf{同じリプルを許すなら，$f$を10倍にすれば$L$も$C$も10分の1で済む}（\FIGref{fig5.10}）。インダクタとキャパシタは電源の体積の大半を占める部品だから，
\index{こうしゅうはか@高周波化}\textbf{高周波化}はそのまま電源の小型化・軽量化につながる。単位体積あたりに変換できる電力
（\index{ぱわーみつど@パワー密度}\textbf{パワー密度}）を高める最も効く手段である。

\begin{figure}[ht]
\begin{center}
\includegraphics[clip]{figures/fig5.10.eps}
\end{center}
...
%%======================================================================
\begin{章末問題}
\item \label{ex5.1}入力$24$\,Vから$12$\,V・$3$\,Aを作る降圧コンバータを設計する。スイッチング周波数は$f = 200$\,kHzとする。
\begin{enumerate}
...
\item \label{ex5.2}バッテリー$12$\,Vから$48$\,V・$0.5$\,Aを作る昇圧コンバータ（$f = 100$\,kHz，$L = 100\,\mathrm{\mu H}$）について，損失を無視して答えよ。
\begin{enumerate}
...
\item \label{ex5.3}昇降圧コンバータについて，オン期間とオフ期間の等価回路を描き，ボルト秒平衡から式\ref{eq5.18}を導け。
また，入力$12$\,Vから大きさ$12$\,V，$5$\,V，$24$\,Vの負電圧を得るためのデューティ比をそれぞれ求めよ。

\item \label{ex5.4}降圧コンバータがCCMを保つ条件を負荷抵抗$R$で表そう。
\begin{enumerate}
...
\item \label{ex5.5}ダイオードの順方向電圧降下$V_F$を無視しない場合の降圧コンバータを考える（それ以外の仮定はそのまま）。
\begin{enumerate}
...
\item \label{ex5.6}
（LTspiceで確かめよ）\rev{配布モデル\texttt{chapter05/buck\_converter.asc}}の降圧コンバータ（$V_{\mathrm{in}} = 10$\,V，$D = 0.5$，$f = 1$\,kHz，$L = 30$\,mH，

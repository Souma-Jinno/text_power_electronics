#!/usr/bin/env python3
# fig2.3（第2章）: pn接合と空乏層。構造（固定電荷）→電荷密度→電場→バンドの曲がり
...
fig, axes = plt.subplots(4, 1, figsize=(4.4, 3.75),
...
ax.text(-L + 0.15, 1.25, "p形", fontsize=8, fontproperties=JP)
ax.text(L - 0.55, 1.25, "n形", fontsize=8, fontproperties=JP)
ax.text(0, 1.25, "空乏層", ha="center", fontsize=8, fontproperties=JP, color="#555")
...
ax.text(0, -1.08, "固定イオン", ha="center", va="top", fontsize=8,
...
ax.text(0.45, 0.42, r"$+eN_d$", fontsize=8, color=BLUE, ha="center")
ax.text(-0.45, -0.62, r"$-eN_a$", fontsize=8, color=RED, ha="center")
...
ax.text(1.35, -1.15, "電場はn形→p形の向き", fontsize=8, fontproperties=JP,
...
ax.text(0, -0.5, "電位の差がバンドの曲がりになる", ha="center", va="top",

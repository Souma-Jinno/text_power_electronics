#!/usr/bin/env python3
# fig5.5（第5章）: 昇圧コンバータの回路構成と，オン期間・オフ期間の等価回路。
...
def draw_boost(ax, mode, small=False):
    ...
    if mode == "on":
        iarr(ax, xV + 0.06, yT, 0.48, 0)
        iarr(ax, xA, 1.20, 0, -0.44)         # S を下向きに流れる
        iarr(ax, 0.5 * (xV + xA) + 0.45, yB, -0.7, 0)
        iarr(ax, xB + 0.2, yT, 0.75, 0)  # CからRへ
    elif mode == "off":
        iarr(ax, xV + 0.06, yT, 0.48, 0)
        iarr(ax, xD1 + 0.03, yT, 0.5, 0)
        iarr(ax, 0.5 * (xA + xB) - 0.2, yB, -0.7, 0)
    ax.set_xlim(*xlim)

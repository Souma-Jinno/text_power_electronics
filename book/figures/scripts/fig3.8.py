...
ax1.plot(t, i, color=BLUE, lw=1.3, ls="--")   # 電流は破線（電圧と線種で区別）
ax1.text(0.55, V + 0.10, "電圧 $v$", fontsize=8, fontproperties=JP, color=RED)
ax1.text(3.85, I + 0.10, "電流 $i$", fontsize=8, fontproperties=JP, color=BLUE)
ax1.text(0.55, 0.08, "オフ", fontsize=8, fontproperties=JP, color="#555")
ax1.text(3.80, 0.30, "オン", fontsize=8, fontproperties=JP, color="#555")
for x0, x1, lab in [(t1, t3, "$t_{\\mathrm{on}}$"), (t4, t6, "$t_{\\mathrm{off}}$")]:
    ...
    ax1.text((x0 + x1) / 2, 1.36, lab, ha="center", fontsize=8, color="#555")
...
ax2.text(2.1, 1.13, "面積 $=E_{\\mathrm{on}}$", ha="center", fontsize=8,
         fontproperties=JP, color="#a04020")
ax2.text(5.9, 1.13, "面積 $=E_{\\mathrm{off}}$", ha="center", fontsize=8,
         fontproperties=JP, color="#a04020")
ax2.text(4.0, 0.28, "導通損失 $R_{\\mathrm{on}}I^2$", ha="center", fontsize=8,
...
ax2.set_yticks([])
ax2.set_xticks([])
ax2.set_xlabel("時刻 $t$", fontsize=8, fontproperties=JP)
ax2.text(-0.25, 0.72, "$p = vi$", fontsize=8, rotation=90, va="center", ha="right")

fig.tight_layout(h_pad=0.4)
EPS = os.path.expanduser("~/text_power_electronics/book/figures/fig3.8.eps")

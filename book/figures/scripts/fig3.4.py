#!/usr/bin/env python3
# fig3.4（第3章）: MOSFETの出力特性（ドレイン電流-ドレイン・ソース間電圧）。
...
for vgs in [3.0, 3.5, 4.0, 4.5, 5.0]:
    ...
    ax.text(5.08, K * vov**2 / 2, rf"$v_{{GS}}={vgs:.1f}$ V",
            fontsize=8, va="center", color="#333")

# 線形領域（オンで使う領域）の強調
ax.plot(vds[vds < 0.55], K * 3.0 * vds[vds < 0.55], color=RED, lw=2.0, ls="--")
# 注記は曲線のない左上（v_GS=5 V の曲線より上）に置く
ax.annotate("スイッチとして使う領域\n（傾きの逆数がオン抵抗$R_{on}$）",
            xy=(0.32, 5.2), xytext=(0.08, 21.6), va="top", ha="left",
            fontsize=8, fontproperties=JP, color=RED,
            arrowprops=dict(arrowstyle="->", lw=0.8, color=RED,
                            shrinkB=4, relpos=(0.1, 0.0)))
# v_DS = 0.5 V の補助目盛（例題で 6 A / 3.5 A を読む位置）
ax.set_xticks([0.5], minor=True)
ax.set_xticklabels(["0.5"], minor=True)
ax.tick_params(axis="x", which="minor", length=2.5, labelsize=8)
ax.plot([0.5, 0.5], [0, 6.5], ls=":", color="#888", lw=0.7)
ax.text(2.2, 0.5, r"$v_{GS}<V_{th}$では$i_D=0$（オフ）", fontsize=8,
...
ax.set_ylabel("ドレイン電流 $i_D$ 〔A〕", fontsize=8, fontproperties=JP)

...
ax.text(2.0, 2.75, r"電場 $\mathcal{E}$", ha="center", fontsize=8,
...
ax.text(0.28, 1.95, "電子", fontsize=8, fontproperties=JP,
...
ax.text(1.8, 0.3, r"$v_d=\mu\mathcal{E}$", ha="center", va="top", fontsize=8,
        color=BLUE)
ax.text(2.0, -0.2, "(a) ドリフト：電場に押されて流れる\n（×印は結晶との衝突）",
        ha="center", va="top", fontsize=8, fontproperties=JP, color="#555")
ax.set_xlim(-0.3, 4.3)
...
for i in range(46):
    ...
    yi = rng.uniform(0.55, 2.05)
    ax.plot(xi, yi, "o", ms=2.6, color=BLUE)
ax.annotate("", xy=(3.3, 2.6), xytext=(1.4, 2.6),
            arrowprops=dict(arrowstyle="-|>", lw=1.4, color=BLUE))
ax.text(2.35, 2.75, "キャリアの流れ", ha="center", fontsize=8,
        fontproperties=JP, color=BLUE)
# 濃い/薄いは箱の内側下段に置く（下のサブキャプションと重ねない）
ax.text(0.6, 0.1, "濃い", ha="center", va="bottom", fontsize=8,
        fontproperties=JP, color="#555")
ax.text(3.4, 0.1, "薄い", ha="center", va="bottom", fontsize=8,
        fontproperties=JP, color="#555")
ax.text(2.0, -0.2, "(b) 拡散：濃度の高い方から\n低い方へ広がる", ha="center",

#!/usr/bin/env python3
# fig2.2（第2章）: 真性・n型・p型半導体のバンド図とキャリア。
...
def frame(ax, title):
    ...
    ax.set_title(title, fontsize=8, fontproperties=JP, pad=3)
...
ax.text(-0.05, 1.8, r"$E_d$", fontsize=8, va="center", color=RED, ha="right")
for x in (0.8, 1.5, 2.2):
    ax.text(x, 1.8, "+", ha="center", va="center", fontsize=8, color=RED)
electrons(ax, [0.6, 1.2, 1.8, 2.4], 2.15)
holes(ax, [1.5], -0.15)
ax.text(1.5, -1.0, "多数キャリア：電子\n（ドナーが供給）", ha="center", fontsize=8,
        fontproperties=JP, color="#555")
...
ax.text(-0.05, 0.2, r"$E_a$", fontsize=8, va="center", color=BLUE, ha="right")
for x in (0.8, 1.5, 2.2):
    ax.text(x, 0.2, "−", ha="center", va="center", fontsize=8, color=BLUE)
holes(ax, [0.6, 1.2, 1.8, 2.4], -0.15)
electrons(ax, [1.5], 2.15)
ax.text(1.5, -1.0, "多数キャリア：正孔\n（アクセプタが供給）", ha="center", fontsize=8,
        fontproperties=JP, color="#555")
